...
export const VALIDATOR_TYPE_REGEX =
  /@zod\.(?<type>[\w]+){1}(?<customErrors>\([{][\w\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Han} (),'"。、ー「」・：（）※ﾞﾟ　:+*#!§$%&/{}[\]=?~><°^\\-]+[}]\))?(?<validatorPattern>[\w\p{Script=Hiragana}\p{Script=Katakana}\p{Script=Han} (),.'"。、ー「」・：（）※ﾞﾟ　\\:+*#!§$%&/{}[\]=?~><°^\\-]*[)])?/u;
